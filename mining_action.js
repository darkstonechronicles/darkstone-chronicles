...
const MINING_ACTION_TEMPLATE = `
  ...
  <div class="profXpShell">
    <div id="miningBonusBox" class="profBonusCard" style="padding:12px;width:100%;min-height:56px;display:flex;align-items:flex-start;gap:10px;">
  ...
  <div class="profActionCard">
    <div style="display:flex;gap:12px;align-items:center;">
      <div style="display:flex;flex-direction:column;align-items:center;gap:8px;min-width:74px;">
        ...
        <img id="oreImg" src="" alt="Ore" class="profChoiceThumb" style="width:74px;height:74px;border-radius:12px;object-fit:cover;">
      </div>
      <div style="flex:1;">
...
function appendMiningReceipt(save, receipt){
...
function commitMiningTick(save, payload = {}){
  const next = save && typeof save === "object" ? save : {};
  const receipt = {
    id: `mining:${Date.now()}:${Math.random().toString(36).slice(2, 10)}`,
    at: new Date().toISOString(),
    profession: "mining",
    oreId: String(payload.oreId || ""),
    oreName: String(payload.oreName || ""),
  ...
  appendMiningReceipt(next, receipt);
  ...
  void window.DSAuth?.invokeActionJournal?.({
    ...
    sourcePage: "mining_action.html",
    payload: {
      profession: receipt.profession,
      oreId: receipt.oreId,
      oreName: receipt.oreName,
      xp: receipt.xp,
    ...
    console.warn("[mining] action journal failed", error);
  });
}
